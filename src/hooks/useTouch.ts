...
interface UseTouchProps {
  ...
  onPan?: (offsetX: number, offsetY: number, containerWidth?: number, containerHeight?: number, pageWidth?: number, pageHeight?: number) => void;
  enabled?: boolean;
  viewMode?: ViewMode;
...
export const useTouch = ({
  ...
  const handleTouchMove = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (touches.length === 2 && isPinchingRef.current && onZoomIn && onZoomOut) {
      // ピンチズーム処理
      event.preventDefault();
    ...
    } else if (touches.length === 1 && isZoomed && onPan && panStartRef.current) {
      // ズーム状態でのパン操作
      event.preventDefault();
      const touch = touches[0];
  ...
  // タップ処理
  const handleTap = useCallback((touchPoint: TouchPoint, element: HTMLDivElement) => {
    ...
    if (viewMode === 'single') {
      ...
      // 見開き表示時のタップナビゲーション
      const shouldGoNext = (readingDirection === 'rtl' && tapX < centerX) || 
