...
test.describe("漫画本PDFビューワー E2E テスト", () => {
  ...
  test("PDFファイル読み込みと基本操作", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /PDFビューワー/ }),
    ).toHaveAttribute("aria-label", /ページ 1 \/ 6/);
  ...
  test("ページナビゲーション", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /PDFビューワー/ }),
    ).toHaveAttribute("aria-label", /ページ 2 \/ 6/);
    ...
    await expect(
      page.getByRole("button", { name: /PDFビューワー/ }),
    ).toHaveAttribute("aria-label", /ページ 6 \/ 6/);
  ...
  test("表示方式の切り替え（単ページ ⇔ 見開き）", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /PDFビューワー/ }),
    ).toHaveAttribute("aria-label", /見開き表示/);
    ...
    await expect(
      page.getByRole("button", { name: /PDFビューワー/ }),
    ).toHaveAttribute("aria-label", /単ページ表示/);
    await expect(page.getByRole("img", { name: "ページ 2" })).toBeVisible();
  });
