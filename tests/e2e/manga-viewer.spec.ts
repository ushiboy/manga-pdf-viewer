...
test.describe('漫画本PDFビューワー E2E テスト', () => {
  ...
  test('初期画面の表示確認', async ({ page }) => {
    ...
    // 初期画面の要素確認
    await expect(page.getByRole('heading', { name: '漫画本PDFビューワー' })).toBeVisible();
  ...
  test('PDFファイル読み込みと基本操作', async ({ page }) => {
    ...
    await expect(page.getByRole('main')).toHaveAttribute('aria-label', /ページ 1 \/ 6/);
  ...
  test('ページナビゲーション', async ({ page }) => {
    ...
    await expect(page.getByRole('main')).toHaveAttribute('aria-label', /ページ 2 \/ 6/);
    ...
    await expect(page.getByRole('main')).toHaveAttribute('aria-label', /ページ 6 \/ 6/);
  ...
  test('設定パネルの全機能確認', async ({ page }) => {
    ...
    await expect(page.getByRole('radio', { name: '右→左（日本語）' })).toBeVisible();
    await expect(page.getByRole('radio', { name: '左→右（英語）' })).toBeVisible();
    ...
    // 設定リセットボタンの確認
    await expect(page.getByRole('button', { name: '🔄 設定を初期値に戻す' })).toBeVisible();
  ...
  test('PWA機能確認', async ({ page }) => {
    ...
    if (await notification.isVisible()) {
      await expect(notification).toContainText('アプリがオフラインで利用可能になりました');
  ...
  async function closePWANotification(page) {
    try {
      ...
      // 通知が消えるまで待機
      await page.locator('[role="alert"]').waitFor({ state: 'hidden', timeout: 3000 });
    } catch (error) {
      // 通知が表示されない場合は何もしない
    }
  }
